...
import { clientesAPI, veiculosAPI, maskPhone, TIPO_LABELS, TIPO_COLORS, type Cliente, type Veiculo } from '../api'
            ...
            <tbody>
              {filtrados.length ? filtrados.map(c => (
                <tr key={c.id}>
                  ...
                  <td><span className="badge bbl" style={{ cursor: 'pointer' }} onClick={() => navigate(`/clientes/${c.id}/edit`)}>{c.totalVeiculos} veíc.</span></td>
...
function ClienteForm() {
  ...
  useEffect(() => {
    ...
    veiculosAPI.listarPorCliente(editId).then(setVeiculos)
  }, [editId])
  ...
  return (
    <Layout title={title} pageId="clientes" actions={actions}>
      ...
      {editId && (
        <div className="card">
          <div className="chd">
            <div className="ctitle">Veículos deste cliente ({veiculos.length})</div>
            <button className="btn btn-g sm" onClick={() => navigate(`/veiculos/new?clienteId=${editId}`)}>+ Adicionar veículo</button>
          </div>
          <div className="cbd" style={{ paddingTop: veiculos.length ? 0 : 14 }}>
            {veiculos.length ? veiculos.map(v => {
              const col = TIPO_COLORS[v.tipo] || 'var(--tx2)'
              return (
