...
public class OrdemController {
    ...
    public OrdemController(OrdemServicoService service, EmailService emailService){
        this.service=service; this.emailService=emailService;
    }
    @GetMapping public List<OrdemResponse> listar(){return service.listar();}
    @GetMapping(params="status") public List<OrdemResponse> listarPorStatus(@RequestParam String status){return service.listarPorStatus(status);}
    @GetMapping("/{id}") public OrdemResponse buscar(@PathVariable Long id){return service.buscarPorId(id);}
    @PostMapping public ResponseEntity<OrdemResponse> criar(@RequestBody OrdemRequest req){
        return ResponseEntity.status(HttpStatus.CREATED).body(service.criar(req));
    }
    @PutMapping("/{id}") public OrdemResponse atualizar(@PathVariable Long id,@RequestBody OrdemRequest req){return service.atualizar(id,req);}
